...
from pytgs.io import get_num_signals, get_file_prefix
from pytgs.paths import Paths
...
class TGSAnalyzer:
    ...
    def fit_signal(self, pos_file: str, neg_file: str) -> Tuple[pd.DataFrame, List[List[float]]]:
        (start_idx, start_time, grating_spacing,
         ...
         _fft_full, _lorentzian_curve) = tgs_fit(
            self.config, pos_file, neg_file, **self.config['tgs']
        )
    ...
    def fit(self, show: bool = True) -> None:
        ...
        for study_name, i in self.idxs:
            ...
            try:
                df, signal = self.fit_signal(pos_file, neg_file)
